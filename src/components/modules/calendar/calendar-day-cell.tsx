...
export function CalendarDayCell({ date, day, isToday, entries, onClick, onEdit }: CalendarDayCellProps) {
  return (
    <div
      ...
      <span
        className={[
          ...
          isToday ? 'bg-primary text-primary-foreground' : 'text-neutral-700 dark:text-neutral-300'
        ].join(' ')}>
        {day}
      </span>
        ...
        {entries.map((entry) => (
          <button
            ...
            className={[
              ...
              entry.blocked ? 'bg-secondary-foreground/20 text-secondary-foreground' : 'bg-primary/10 text-primary'
            ].join(' ')}>
            {entry.title}
          </button>
        ))}
