...
export class SessionRepositoryImpl implements SessionRepository {
  ...
  async createSession({ userId, token }: Session): Promise<void> {
    ...
    await this.cookieStorage.set('auth_session', sessionEncrypted, this.COOKIE_OPTIONS);

    await this.cookieStorage.set('auth_token', tokenEncrypted, this.COOKIE_OPTIONS);
  }
